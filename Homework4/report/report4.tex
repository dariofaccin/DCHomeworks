...
\begin{figure}[H]
	...
	\draw [->] (start) --node[label={[label distance=0.2cm]270:$T_{bit}$}, label={[label distance=0.1cm]90:$b_\ell$}]{} (encoder);
...
Defining the DFT of the vector $\mathbf{g}_C$ as $\mathbf{G}_C = \mathbf{F}_\mathcal{M}\mathbf{g}_C$ and using equation \ref{property}, we can express the demodulator output as 
...
\begin{equation}
K_i = [\mathbf{F}]_{i,i} = \frac{1}{\mathcal{G}_{C,i}} \quad\quad\quad i=0,\dots \mathcal{M}-1
\end{equation}
